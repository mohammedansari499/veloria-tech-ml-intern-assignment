...
To demonstrate that both tasks were executed successfully, I have included a file named:

**Terminal Execution of Tasks 1 & 2.txt**

This file contains the complete terminal output generated while running both programs.

For Task 1, the output shows the scraper accessing HowStat, processing the latest completed Test matches for England and India, extracting match details, identifying the top scorer from each scorecard, and generating the final `match_data.csv` dataset. :contentReference[oaicite:0]{index=0}

For Task 2, the output shows the machine learning workflow from data loading and preprocessing through model training, evaluation, and feature importance analysis. The results include the Accuracy Score, F1 Score, Confusion Matrix, Classification Report, and final model summary. The model achieved an accuracy of **96.65%** with a weighted **F1 Score of 0.9665** on the test dataset. :contentReference[oaicite:1]{index=1}

The execution log has been included to provide transparency, demonstrate successful execution of the code, and allow reviewers to verify the outputs generated by each task without needing to run the programs immediately.
